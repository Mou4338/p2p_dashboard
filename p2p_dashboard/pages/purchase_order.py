...
def show():
    st.markdown('<div class="section-header">📦 Purchase Order (PO) — T-Code: ME21N</div>', unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with col4:
            search = st.text_input("🔍 Search PO Number / Material")
        ...
        def color_status(val):
            c = {"Open":"background-color:#FFF3CD;color:#856404",
        ...
        st.dataframe(filtered.style.applymap(color_status, subset=["Status"]),
                     use_container_width=True, height=400)

        total_val = filtered["PO Value (₹)"].sum()
    ...
    # ── Tab 2: Create PO ──────────────────────────────────────────────────────
    with tab2:
        st.markdown("#### Simulate: Create Purchase Order (ME21N)")
        st.info("💡 Simulates the SAP ME21N PO creation screen — Header + Line Item data")
        ...
        with st.form("po_form"):
            ...
            with c2:
                pur_org  = st.selectbox("Purchasing Org.", ["1000 – Central Purch.","2000 – Regional"])
            ...
            with c5:
                ...
                net_price = st.number_input("Net Price (₹)", min_value=1.0, value=150.0, step=0.5)
            with c6:
                plant     = st.selectbox("Plant", PLANTS)
                sloc      = st.selectbox("Storage Loc.", ["0001 – Main","0002 – Production","0003 – QC"])
            with c7:
    ...
    with tab3:
        ...
        c1, c2 = st.columns(2)
        with c1:
            vend_val = po.groupby("Vendor Name")["PO Value (₹)"].sum().reset_index()
            fig = px.pie(vend_val, names="Vendor Name", values="PO Value (₹)", hole=0.4,
                         title="PO Value by Vendor",
                         color_discrete_sequence=px.colors.sequential.Blues_r)
            st.plotly_chart(fig, use_container_width=True)
        with c2:
            stat_val = po.groupby("Status")["PO Value (₹)"].sum().reset_index()
            fig2 = px.bar(stat_val, x="Status", y="PO Value (₹)", title="PO Value by Status",
                          color="Status",
                          color_discrete_map={"Open":"#FFC107","Partially Delivered":"#2E75B6",
                                              "Fully Delivered":"#28A745","Closed":"#6C757D"})
            fig2.update_layout(showlegend=False)
            st.plotly_chart(fig2, use_container_width=True)

        c3, c4 = st.columns(2)
        with c3:
            mat_val = po.groupby("Description")["PO Value (₹)"].sum().nlargest(6).reset_index()
            fig3 = px.bar(mat_val, x="PO Value (₹)", y="Description", orientation='h',
        ...
        with c4:
            plant_val = po.groupby("Plant")["PO Value (₹)"].sum().reset_index()
            fig4 = px.bar(plant_val, x="Plant", y="PO Value (₹)", title="PO Value by Plant",
                          color="PO Value (₹)", color_continuous_scale="Blues")
