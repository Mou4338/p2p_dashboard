...
def show():
    st.markdown('<div class="section-header">Overview — Procure-to-Pay Dashboard</div>', unsafe_allow_html=True)
    st.caption("ABC Manufacturing Ltd.  |  SAP MM Module  |  FY 2024-25")
    ...
    with k1:
        st.metric("Purchase Requisitions", len(pr), delta=f"{len(pr[pr.Status=='Open'])} Open")
    with k2:
        st.metric("Purchase Orders", len(po), delta=f"Rs {po['PO Value (Rs)'].sum()/1e5:.1f}L value")
    with k3:
        st.metric("Goods Receipts Posted", len(gr), delta=f"Rs {gr['Value (Rs)'].sum()/1e5:.1f}L received")
    with k4:
    ...
    for col, (label, code) in zip(cols, steps):
        with col:
            if label == "arrow":
                st.markdown("<div style='text-align:center;font-size:20px;padding-top:18px;color:#2E75B6'>&#8594;</div>", unsafe_allow_html=True)
            else:
                st.markdown(f"""
                <div class='process-step'>
                    <div style='font-size:11px'>{label}</div>
                    <div style='font-size:10px;color:#2E75B6;font-weight:700;margin-top:4px'>{code}</div>
    ...
    with col2:
        ...
        po_vend = po.groupby("Vendor Name")["PO Value (Rs)"].sum().nlargest(5).reset_index()
        fig2 = px.bar(po_vend, x="PO Value (Rs)", y="Vendor Name", orientation='h',
                      color="PO Value (Rs)", color_continuous_scale=["#9DC3E6", "#1F4E79"])
        fig2.update_layout(height=300, margin=dict(t=10, b=10, l=10, r=10),
                           coloraxis_showscale=False,
                           yaxis=dict(categoryorder='total ascending'))
        fig2.update_traces(texttemplate='Rs %{x:,.0f}', textposition='outside')
    ...
    with col3:
        ...
        fig3.update_layout(height=300, margin=dict(t=10, b=10, l=10, r=10), showlegend=False)
        st.plotly_chart(fig3, use_container_width=True)

    with col4:
        st.markdown("#### GR — Ordered vs Received Quantity (Top 8)")
    ...
    with col5:
        st.markdown("**Blocked Invoices — Pending Release (MRBR)**")
        blocked_inv = inv[inv.Status == "Blocked"][
            ["Invoice No", "Vendor Name", "Total Amt (Rs)", "Block Reason", "Due Date"]
        ]
        st.dataframe(blocked_inv.reset_index(drop=True), use_container_width=True, height=200)

    with col6:
        st.markdown("**Overdue Vendor Payments**")
        overdue_pay = pay[pay.Status == "Overdue"][
            ["Payment Doc", "Vendor Name", "Amount (Rs)", "Due Date", "Method"]
        ]
    ...
    with f1:
        st.markdown(f"""<div class='kpi-card'>
            <div class='kpi-value'>Rs {po['PO Value (Rs)'].sum()/1e5:.1f}L</div>
            <div class='kpi-label'>Total PO Value</div></div>""", unsafe_allow_html=True)
    with f2:
        st.markdown(f"""<div class='kpi-card'>
            <div class='kpi-value'>Rs {gr['Value (Rs)'].sum()/1e5:.1f}L</div>
            <div class='kpi-label'>Goods Received Value</div></div>""", unsafe_allow_html=True)
    with f3:
        st.markdown(f"""<div class='kpi-card'>
            <div class='kpi-value'>Rs {inv['Total Amt (Rs)'].sum()/1e5:.1f}L</div>
            <div class='kpi-label'>Total Invoice Value</div></div>""", unsafe_allow_html=True)
    with f4:
        paid_amt = pay[pay.Status == "Cleared"]["Amount (Rs)"].sum()
        st.markdown(f"""<div class='kpi-card'>
            <div class='kpi-value'>Rs {paid_amt/1e5:.1f}L</div>
