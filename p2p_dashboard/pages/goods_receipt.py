...
def show():
    st.markdown('<div class="section-header">🚚 Goods Receipt (GR) — T-Code: MIGO</div>', unsafe_allow_html=True)
    ...
    # ── Tab 1: GR Register ────────────────────────────────────────────────────
    with tab1:
        st.markdown("#### Goods Receipt Documents — MB51 / MIGO Display")
        ...
        st.dataframe(filtered.style.apply(highlight_shortfall, axis=1),
                     use_container_width=True, height=400)

        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Total GR Docs", len(filtered))
        m2.metric("Total Qty Received", f"{filtered['Qty Received'].sum():,}")
        m3.metric("Total Value (₹)", f"₹{filtered['Value (₹)'].sum():,.0f}")
    ...
    # ── Tab 2: Post GR ────────────────────────────────────────────────────────
    with tab2:
        st.markdown("#### Simulate: Post Goods Receipt (MIGO)")
        st.info("💡 Simulates MIGO — A01 Goods Receipt against Purchase Order")
        ...
        with st.form("gr_form"):
            ...
            with c1:
                ...
                ref_doc     = st.selectbox("Reference Document Type", ["R01 – Purchase Order","R02 – Order","R10 – Delivery"])
            ...
            with c3:
                storage_loc = st.selectbox("Storage Location", ["0001 – Main Store","0002 – Prod. Store","0003 – QC Store"])
        ...
        if submitted:
            ...
            if qty_received <= qty_ordered:
                st.success(f"✅ Goods Receipt posted! Material Document: **{mat_doc}**")
                c1, c2 = st.columns(2)
                with c1:
                    st.markdown(f"""
                    **Material Document:**
                ...
                with c2:
                    ...
                    **Stock & Accounting Impact:**
                    ...
                    """)
                if qty_received < qty_ordered:
                    st.warning(f"⚠️ Partial delivery: {qty_ordered - qty_received} units pending from vendor.")
            else:
                st.error("❌ Received qty cannot exceed ordered qty. Please check.")

    # ── Tab 3: Stock Overview ─────────────────────────────────────────────────
    with tab3:
        st.markdown("#### Stock Overview — MMBE")
        st.markdown("Simulated current stock positions across all plants:")
        ...
        for mat_id, (name, uom, price) in MATERIALS.items():
            for plant in ["1000 – Mumbai","2000 – Pune"]:
                ...
                stock_rows.append({
                    "Material": mat_id, "Description": name,
                    "Plant": plant, "UOM": uom,
                    "Unrestricted": unr, "QI Stock": qi, "Blocked": blkd,
                    "Total Stock": unr+qi+blkd,
                    "Stock Value (₹)": round((unr+qi)*price, 2)
                })
        stock_df = pd.DataFrame(stock_rows)

        # Summary
        c1, c2, c3 = st.columns(3)
        c1.metric("Total Stock Value", f"₹{stock_df['Stock Value (₹)'].sum():,.0f}")
        c2.metric("Materials in Stock", len(stock_df["Material"].unique()))
        c3.metric("Plants", len(stock_df["Plant"].unique()))

        # Chart
        fig = px.bar(stock_df, x="Description", y=["Unrestricted","QI Stock","Blocked"],
                     barmode="stack", title="Stock by Material & Category",
                     color_discrete_map={"Unrestricted":"#2E75B6","QI Stock":"#FFC107","Blocked":"#DC3545"})
